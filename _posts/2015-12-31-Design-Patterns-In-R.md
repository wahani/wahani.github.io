...
algorithm to find the estimator. The fixed point function, in this case I use
Newton-Raphson algorithm, is defined as:
...
are recomputed in every iteration. In my code this very often comes with the 
...
So far the fixed point framework does not allow to restrict the number of iterations. This is of course something you always want to control. To implement this I rely, again, on closures. This time, however, to model mutable state. To illustrate this, consider the two implementations of a counter:
...
combine the *wrapper pattern* and the *mutability pattern* to modify the
...
Now we can see that the initial version of the algorithm osciliates between 1
...
algorithm anymore. But let's compare a different implementation:
...
And now let's add the number of iterations to the return value of `fp` (without
touching it):
...
Maybe this implementation deserves an own name, like *annotation*, but you can
still see the *wrapper* and *mutability* pattern and it is similar to the
`averageDamp` function. Compared to `fpImp` the logic around a *maximum number
of iterations* has been seperated from the concrete implementation of the
